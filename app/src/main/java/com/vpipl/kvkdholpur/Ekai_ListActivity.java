...
import com.vpipl.kvkdholpur.Adapter.Ikaai_List_Adapter;
...
public class Ekai_ListActivity extends AppCompatActivity {
    ...
    private void executeEkaiListRequest() {
        try {
            if (AppUtils.isNetworkAvailable(act)) {
                new AsyncTask<Void, Void, String>() {
                    ...
                    protected String doInBackground(Void... params) {
                        ...
                        try {
                            ...
                            response = AppUtils.callWebServiceWithMultiParam(act, postParameters, QueryUtils.methodToSelectEkai, TAG);
    ...
    private void getAllActivityListResult(JSONArray jsonArray) {
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                hashMap.put("id", jsonObject.getString("AutoID"));
    ...
    private void showListView() {
        try {
            if (arraylist.size() > 0) {
                adapter = new Ikaai_List_Adapter(act, arraylist);
